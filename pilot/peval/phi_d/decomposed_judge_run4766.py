...
RACE-IMMUNE INSTANCE: byte-identical prompt surface to the smoke-validated
judge state (prompt_sha 47669c03df56..., 8/8 smoke valid) — the sibling file
decomposed_judge.py is being concurrently edited by another agent; this copy
pins the validated constants for the canonical out/judgments.jsonl production
run. All logic identical to decomposed_judge.py @ the 4766 state.

Run:
  CUDA_VISIBLE_DEVICES=4 HF_HOME=/work1/zixuan/cache/huggingface HF_HUB_OFFLINE=1 \
    /work1/zixuan/envs/conda_envs/causalmemagent/bin/python decomposed_judge_run4766.py [--limit N]
"""
import argparse
import json
import os
...
Now do the same for the real pair below. The answer must continue directly from: {"schema":"phi_judge/v0","fields":{"goal":{"verdict":
...
PREFILL = '{"schema":"phi_judge/v0","fields":{"goal":{"instruction_says":'
BATCH = 64
