...
RACE-IMMUNE INSTANCE: byte-identical prompt surface to the extraction state that
produced canonical out/extractions.jsonl (prompt_sha 5200e56eee95..., 532 rows,
full-run 2026-08-10 02:03-02:15; smoke 8/8 valid in out/dev_smoke smoke runs).
The sibling extract_phi.py is being concurrently iterated by another agent; this
copy pins the frozen constants. Logic identical to extract_phi.py @ frozen state.
...
Run:
  CUDA_VISIBLE_DEVICES=4 HF_HOME=/work1/zixuan/cache/huggingface HF_HUB_OFFLINE=1 \
    /work1/zixuan/envs/conda_envs/causalmemagent/bin/python extract_phi_run5200.py [--limit N]
"""
import argparse
import json
import os
...
Now do the same for the real TEXT below. The answer must continue directly from: {"schema":"phi_ir/v0","roles":{"subject_row":{"status":
...
PREFILL = '{"schema":"phi_ir/v0","roles":{"subject_row":{"status":'
BATCH = 64
