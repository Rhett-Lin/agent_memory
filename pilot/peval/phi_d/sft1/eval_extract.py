#!/usr/bin/env python
"""eval_extract.py — extraction eval for the SFT pilot: base vs LoRA-SFT extractor.
...
retry, validate_ir. vLLM with enable_lora; the base condition generates WITHOUT
a LoRARequest in the same process (same weights state), the SFT condition with
the trained adapter. Canonical-80 base numbers are NOT re-run: they come from
the frozen out/extractions_v2.jsonl (audited lineage); the SFT condition on the
same 80 keys runs here for direct comparability.

Jobs (one process, one GPU):
  1. base  x test90   (minted P1 test slice, no LoRA)
  2. sft   x test90   (LoRA adapter)
  3. sft   x canon80  (first 80 keys of extractions_v2.jsonl, file order)

Run:
  CUDA_VISIBLE_DEVICES=4 HF_HOME=... HF_HUB_OFFLINE=1 PY eval_extract.py [--limit N]
"""
import argparse
import json
import os
...
ADAPTER = "/work1/zixuan/checkpoints/agent_memory/phi_sft/lora_p1_sft1"
CANON80 = 80
EVALDIR = HERE / "eval"
...
def main():
    ...
    ap.add_argument("--only", default="")          # e.g. "sft_test90" to run one job
    args = ap.parse_args()
    ...
    canon = v2[:CANON80]
    jobs_t90 = [{"key": "t90:%s" % r["pair_id"], "kind": r["kind"], "text": r["text"]}
                for r in test_rows]
    jobs_c80 = [{"key": r["key"], "kind": r["kind"], "text": r["text"]} for r in canon]
    if args.limit:
        jobs_t90, jobs_c80 = jobs_t90[:args.limit], jobs_c80[:args.limit]
    ...
    lora_req = LoRARequest("p1sft1", 1, ADAPTER)

    plan = [("base_test90", jobs_t90, None),
            ("sft_test90", jobs_t90, lora_req),
